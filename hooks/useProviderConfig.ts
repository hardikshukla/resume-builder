...
export function useProviderConfig(): ProviderConfig {
  ...
  const handleOpenrouterKey = useCallback((v: string) => {
    setOpenrouterKey(v);
    sessionStorage.setItem(KEYS.openrouterKey, v);
  }, []);
  ...
  const handleOpenrouterModel = useCallback((v: string) => {
    setOpenrouterModel(v);
    sessionStorage.setItem(KEYS.openrouterModel, v);
  }, []);
